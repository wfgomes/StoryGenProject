...
tf.compat.v1.app.flags.DEFINE_boolean("is_train", True, "Set to False to inference.")
tf.compat.v1.app.flags.DEFINE_integer("symbols", 10000, "vocabulary size.")
tf.compat.v1.flags.DEFINE_integer("embed_units", 200, "Size of word embedding.")
tf.compat.v1.flags.DEFINE_integer("units", 512, "Size of each model layer.")
tf.compat.v1.flags.DEFINE_integer("layers", 2, "Number of layers in the model.")
tf.compat.v1.flags.DEFINE_integer("batch_size", 128, "Batch size to use during training.")
tf.compat.v1.flags.DEFINE_string("data_dir", "./data", "Data directory")
tf.compat.v1.flags.DEFINE_string("train_dir", "./train", "Training directory.")
tf.compat.v1.flags.DEFINE_integer("per_checkpoint", 1000, "How many steps to do per checkpoint.")
tf.compat.v1.flags.DEFINE_integer("inference_version", 0, "The version for inferencing.")
tf.compat.v1.flags.DEFINE_integer("triple_num", 10, "max number of triple for each query")
tf.compat.v1.flags.DEFINE_boolean("log_parameters", True, "Set to True to show the parameters")
tf.compat.v1.flags.DEFINE_string("inference_path", "", "Set filename of inference, default isscreen")
...
def build_vocab(path, data):
    ...
    embed = np.array(embed, dtype=object)
...
def inference(model, sess, dataset):
    ...
    output_file = open("./output_" + str(inference_version) + ".txt", "w")
...
# data_train = load_data(data_dir, 'train')
# data_dev = load_data(data_dir, 'val')
# data_test = load_data(data_dir, 'test')
# vocab, embed, vocab_dict = build_vocab(data_dir, data_train)
# relation = load_relation(data_dir)
