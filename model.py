...
class IEMSAModel(object):
    def __init__(self,
        ...
        self.posts_1 = tf.compat.v1.placeholder(tf.string, shape=(None, None))
        self.posts_2 = tf.compat.v1.placeholder(tf.string, shape=(None, None))
        self.posts_3 = tf.compat.v1.placeholder(tf.string, shape=(None, None))
        self.posts_4 = tf.compat.v1.placeholder(tf.string, shape=(None, None))

        self.entity_1 = tf.compat.v1.placeholder(tf.string, shape=(None,None,None,3))
        self.entity_2 = tf.compat.v1.placeholder(tf.string, shape=(None,None,None,3))
        self.entity_3 = tf.compat.v1.placeholder(tf.string, shape=(None,None,None,3))
        self.entity_4 = tf.compat.v1.placeholder(tf.string, shape=(None,None,None,3))

        self.entity_mask_1 = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None))
        self.entity_mask_2 = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None))
        self.entity_mask_3 = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None))
        self.entity_mask_4 = tf.compat.v1.placeholder(tf.float32, shape=(None, None, None))

        self.posts_length_1 = tf.compat.v1.placeholder(tf.int32, shape=(None))
        self.posts_length_2 = tf.compat.v1.placeholder(tf.int32, shape=(None))
        self.posts_length_3 = tf.compat.v1.placeholder(tf.int32, shape=(None))
        self.posts_length_4 = tf.compat.v1.placeholder(tf.int32, shape=(None))

        self.responses = tf.compat.v1.placeholder(tf.string, shape=(None, None))
        self.responses_length = tf.compat.v1.placeholder(tf.int32, shape=(None))

        self.epoch = tf.compat.v1.Variable(0, trainable=False, name='epoch')
        self.epoch_add_op = self.epoch.assign(self.epoch + 1)

        if is_train:
            self.symbols = tf.compat.v1.Variable(vocab, trainable=False, name="symbols")
        else:
            self.symbols = tf.compat.v1.Variable(np.array(['.']*num_symbols), name="symbols")
        self.symbol2index = tf.compat.v1.lookup.StaticHashTable(tf.compat.v1.lookup.KeyValueTensorInitializer(self.symbols,
        ...
        if embed is None:
            self.embed = tf.compat.v1.get_variable('embed', [num_symbols, num_embed_units], tf.float32)
        else:
            self.embed = tf.compat.v1.get_variable('embed', dtype=tf.float32, initializer=embed)
        ...
        if use_lstm:
            cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.LSTMCell(num_units)] * num_layers)
        else:
            cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.GRUCell(num_units)] * num_layers)
